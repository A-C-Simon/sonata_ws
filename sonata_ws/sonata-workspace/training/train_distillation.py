...
def gpu_weighted_interpolate(features, source_coords, target_coords, k=4):
    """Map features via inverse-distance weighted interpolation (GPU)."""
    from models.diffusion_module import weighted_interpolate
    return weighted_interpolate(features, source_coords, target_coords, k=k)
...
def forward_with_intermediates(denoiser, features, coords, timestep, condition):
    ...
    for i, enc_block in enumerate(denoiser.encoder_blocks):
        ...
        if x.shape[0] > target_num:
            indices = denoiser._fps_indices(x_coords, target_num)
            x = x[indices]
    ...
    for i, dec_block in enumerate(denoiser.decoder_blocks):
        ...
        t_embed = denoiser._upsample_points(t_embed, x_coords, skip_coord)
...
class DistillationTrainer:
    ...
    def compute_losses(self, student_output, teacher_output,
        ...
        # Task loss (always present): MSE on delta prediction vs ground truth
        losses['task'] = self.alpha_task * self.task_loss_fn(
            student_output, target_delta
        )
        ...
        if self.strategy in ('structural', 'all'):
            # Compute denoised x_0 from delta predictions (x0 = x_t + delta)
            if hasattr(self, '_current_noisy'):
                student_x0 = self._current_noisy + student_output
                teacher_x0 = self._current_noisy + teacher_output
    ...
    for step, (teacher_batch, student_batch) in enumerate(pbar):
        ...
        with torch.cuda.amp.autocast(enabled=args.fp16):
            ...
            with torch.no_grad():
                ...
                teacher_cond_mapped = gpu_weighted_interpolate(
                ...
                target_delta = complete_coord - noisy_scan

                # Store for structural loss (x0 = noisy + delta)
                trainer._current_noisy = noisy_scan

                # Teacher prediction (delta) with intermediates
                teacher_pred, teacher_intermediates = forward_with_intermediates(
                    teacher_model.denoiser, noisy_scan, complete_coord, t_per_point,
            ...
            student_cond_mapped = gpu_weighted_interpolate(
                student_cond, student_scan['coord'], complete_coord
            )

            # Student prediction (delta) with intermediates
            student_pred, student_intermediates = forward_with_intermediates(
                student_model.denoiser, noisy_scan, complete_coord, t_per_point,
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help='SemanticKITTI voxel size (match boost GT, default 0.1)',
    )
    parser.add_argument(
        '--gt_subdir', type=str, default='ground_truth',
        help='GT subfolder under each data_path_* (e.g. ground_truth_v2)',
    )
    parser.add_argument(
        '--gt_name_suffix', type=str, default='',
        help='GT filename suffix before .npz (e.g. _v2)',
    )
...
def main():
    ...
    student_train = SemanticKITTI(
        ...
        gt_subdir=args.gt_subdir, gt_name_suffix=args.gt_name_suffix,
    )
