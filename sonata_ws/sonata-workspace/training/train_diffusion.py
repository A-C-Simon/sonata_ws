...
def parse_args():
    ...
    parser.add_argument(
        ...
        help='Path to SemanticKITTI dataset root (sequences/, GT subdir/)'
    )
    parser.add_argument(
        '--gt_subdir', type=str, default='ground_truth',
        help='GT subfolder under data_path (e.g. ground_truth_v2 for map_from_scans_boost_v2)'
    )
    parser.add_argument(
        '--gt_name_suffix', type=str, default='',
        help="Suffix in GT filename before .npz (e.g. _v2 → 000000_v2.npz)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help='Voxel size for partial+GT (match map_from_scans_boost_v2 / BOOST.voxel_size)'
    )
    parser.add_argument(
        '--max_points', type=int, default=20000,
        help=(
            'Max points per sample after voxelize (partial + GT each capped here). '
            'Inference uses --gt-npz to derive target from GT voxels.'
        ),
    )
        ...
        class PrecomputedModel(torch.nn.Module):
            ...
            def forward(self, partial_scan, complete_scan, complete_batch=None,
                ...
                for b in range(batch_size):
                    ...
                    t_pp = torch.full((coords_b.shape[0],), t[b].item(), device=dev, dtype=torch.long)
                    pred_delta = self.denoiser(noisy_b, coords_b, t_pp, {'features': cond_b})
                    target_delta = coords_b - noisy_b
                    sample_loss = F.mse_loss(pred_delta, target_delta) / batch_size
