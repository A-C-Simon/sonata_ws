...
from models.diffusion_module_2 import SceneCompletionDiffusion
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Optional sequence override, e.g. --sequences 00 (default: split list)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--max_points', type=int, default=4000,
        help='Max points per partial/complete after voxelization (reduce to avoid OOM)'
    )
    ...
    parser.add_argument(
        ...
        help=(
            "Conditioning encoder backend. "
            "'sonata' uses the Sonata encoder; "
            "'dummy' disables Sonata and uses zero conditioning (smoke-test)."
        ),
    )
    parser.add_argument(
...
def build_model(args):
    ...
    if getattr(args, 'precomputed', False):
        ...
        from models.diffusion_module_2 import DenoisingNetwork, DiffusionScheduler
    ...
    if getattr(args, 'encoder_backend', 'sonata') == 'dummy':
        print("Building model (dummy conditioning; no Sonata dependencies)...")

        class _DummyEncoder(torch.nn.Module):
            def forward(self, point_dict):
                return point_dict

        class _DummyConditionExtractor(torch.nn.Module):
            out_dim = 256

            def forward(self, point_dict):
                n = int(point_dict["coord"].shape[0])
                dev = point_dict["coord"].device
                return torch.zeros((n, 256), device=dev, dtype=torch.float32), {}

        model = SceneCompletionDiffusion(
            encoder=_DummyEncoder(),
            condition_extractor=_DummyConditionExtractor(),
    ...
    # Full model with Sonata encoder (lazy import to avoid hard HF dependency)
...
def main():
    ...
    train_dataset = SemanticKITTI(
        ...
        sequence_scan_end=args.max_scans_per_sequence,
        use_precomputed=use_precomputed,
        # This dataset version doesn't have debug_checks flag; keep prints in training.
    )
    ...
    val_dataset = SemanticKITTI(
        ...
        gt_name_suffix=args.gt_name_suffix,
        augmentation=False,
        sequence_ids=args.sequences if args.sequences is not None else None,
        sequence_scan_start=0,
        sequence_scan_end=args.max_scans_per_sequence,
        use_precomputed=use_precomputed,
    )
