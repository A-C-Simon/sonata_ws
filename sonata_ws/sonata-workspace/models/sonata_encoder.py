...
class SonataEncoder(nn.Module):
    ...
    def _load_pretrained(
        ...
        try:
            ...
            # Import Sonata model (requires https://github.com/facebookresearch/sonata)
            try:
                ...
                print("Sonata not installed as package, trying ./sonata fallback...")
                import sys
                repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
                fallback = os.path.join(repo_root, "sonata")
                sys.path.insert(0, fallback)
                try:
                    from model import PointTransformerV3
                except ImportError as e_local:
                    raise ImportError(
                        "PointTransformerV3 not found. Install Sonata in this env, e.g. "
                        "clone https://github.com/facebookresearch/sonata and run "
                        "`pip install -e .` there (needs spconv matching your CUDA), "
                        f"or place the repo under ./sonata next to the project. "
                        f"Import errors: {e_pkg}; {e_local}"
                    ) from e_local
                model = PointTransformerV3.from_pretrained(
                    model_id, **custom_config
                )
