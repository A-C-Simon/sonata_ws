...
class SonataTransformerBlock(nn.Module):
    ...
    def _find_neighbors(self, coords: torch.Tensor) -> torch.Tensor:
        ...
        IMPORTANT: avoid O(N^2) GPU memory (torch.cdist) for large N.
        Uses CPU cKDTree and returns neighbor indices on the original device.
        """
        N = int(coords.shape[0])
        if N == 0:
            return torch.zeros((0, self.num_neighbors), dtype=torch.long, device=coords.device)

        k = min(self.num_neighbors + 1, N)
        coords_np = coords.detach().float().cpu().numpy()
        tree = cKDTree(coords_np)
        _, idx = tree.query(coords_np, k=k)
        idx = idx[:, 1:] if k > 1 else idx.reshape(-1, 1)

        idx_t = torch.from_numpy(idx.astype(np.int64)).to(coords.device)
        if idx_t.shape[1] < self.num_neighbors:
            pad = idx_t[:, -1:].expand(-1, self.num_neighbors - idx_t.shape[1])
            idx_t = torch.cat([idx_t, pad], dim=1)
        return idx_t
