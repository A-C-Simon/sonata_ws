...
def load_poses(
    calib_path: str, poses_path: str, show_progress: bool = True
) -> list:
    ...
    with open(poses_path) as f:
        line_iter = tqdm(f, desc="Loading poses", unit="pose") if show_progress else f
...
def voxelize_numpy(points: np.ndarray, voxel_size: float) -> np.ndarray:
    ...
    if n <= _VOXEL_NUMPY_CHUNK:
        voxel_coords = np.floor(points / voxel_size).astype(np.int32)
        unique_voxels = np.unique(voxel_coords, axis=0)
        return unique_voxels.astype(np.float32) * voxel_size + voxel_size / 2

    parts = []
    for start in range(0, n, _VOXEL_NUMPY_CHUNK):
        chunk = points[start : start + _VOXEL_NUMPY_CHUNK]
        voxel_coords = np.floor(chunk / voxel_size).astype(np.int32)
        unique_voxels = np.unique(voxel_coords, axis=0)
        parts.append(unique_voxels.astype(np.float32) * voxel_size + voxel_size / 2)
...
def voxelize(points: np.ndarray, voxel_size: float, backend: str = "numpy") -> np.ndarray:
    """Voxelize point cloud. backend: 'numpy' | 'open3d' | 'torch' | 'auto' | 'auto_cuda'."""
    if backend == "auto_cuda":
        try:
            import torch

            if torch.cuda.is_available():
                return voxelize_torch(points, voxel_size, device="cuda")
        except ImportError:
            pass
        try:
...
def main():
    ...
    parser.add_argument(
        ...
        default="numpy",
        help="Voxelization: numpy, open3d (faster CPU), torch (GPU), auto (open3d if import ok)",
    )
