...
CKPT_PATH = WORK_DIR / "checkpoints" / "diffusion_v2gt_finetune_mixed_scaffold" / "best.pth"
RESULTS_DIR = WORK_DIR / "results" / "apr17_morning"
RESULTS_PATH = RESULTS_DIR / "lidpm_protocol_baseline.json"
NUM_FRAMES = 50
DEVICE = "cuda"
MAX_PTS = 20000
SEED = 42
...
# ---- LiDPM-protocol scaffold (C3 experiment) ----
def scaffold_input_lidpm_protocol(fr, max_pts=MAX_PTS):
    """LiDPM/SDEdit analogue: scaffold IS the partial LiDAR scan (no duplication,
    no jitter, no crop). Tests whether decoupling the scaffold from the partial-scan
    input matters empirically vs noising the input directly."""
    lidar = fr["lidar_coords"]  # already centered, already 20k points (prevoxelized)
    if lidar.shape[0] > max_pts:
        idx = np.random.choice(lidar.shape[0], max_pts, replace=False)
        lidar = lidar[idx]
    elif lidar.shape[0] < max_pts:
        n_needed = max_pts - lidar.shape[0]
        pad_idx = np.random.choice(lidar.shape[0], n_needed, replace=True)
        lidar = np.concatenate([lidar, lidar[pad_idx]], axis=0)
    return lidar.astype(np.float32)


def variant_LiDPM_protocol(model, fr):
    """Scaffold = partial LiDAR scan. SDEdit/LiDPM analogue: take input, noise it
    to t=200, predict x_0 in one step. No GT-coordinate prior."""
    scaffold = scaffold_input_lidpm_protocol(fr)
    if scaffold.shape[0] < 64:
        return None, None
    point_dict = make_point_dict(fr["lidar_coords"])
    target = torch.from_numpy(scaffold).float().to(DEVICE)
...
VARIANTS = [
    # C3: head-to-head between our deployment scaffold (ego-bbox) and
    # LiDPM/SDEdit-protocol scaffold (raw partial scan, no duplication/jitter).
    # If LiDPM_protocol >> our recipe, the decoupling claim is empirically defended.
    ...
    ("LiDPM_protocol_lidiff_crop",
     variant_LiDPM_protocol, "lidiff", "torch"),
    ("LiDPM_protocol_lidiff_match_kdtree",
     variant_LiDPM_protocol, "lidiff", "kdtree"),
]
