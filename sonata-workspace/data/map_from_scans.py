...
def generate_sequence_map(
    ...
    for seq in sequences:
        ...
        if save_only_map_world:
            # Full-sequence aggregation: one map_world.npz per seq (saves disk, for LiDAR baseline).
            all_points = []
            for i, (pose, scan_file) in enumerate(
                tqdm(list(zip(poses, scan_files)), desc=f"Sequence {seq}", leave=False)
            ):
                ...
                points_homo = np.hstack([points[:, :3], ones])
            ...
            np.savez(os.path.join(gt_seq_dir, "map_world.npz"), points=map_voxel.astype(np.float32))
            print(f"Saved map_world only for sequence {seq} (~{map_voxel.nbytes / 1e9:.2f} GB).")
            continue

        # Sliding window (from main): per-scan .npz, avoids OOM with dense Depth Pro clouds.
        WINDOW = 25
